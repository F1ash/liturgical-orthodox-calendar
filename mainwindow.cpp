...
void MainWindow::initialSettingsDock ()
{
    ...
    settingsDock->hide();
    connect(settingsWidget, SIGNAL(settingsApplied()),
            this, SLOT(reloadCalendar()));
}
void MainWindow::initNetworkStuff()
{
    netManager = new NetworkManager(this);
    connect(netManager, SIGNAL(cacheChecked()),
            this, SLOT(initAppWidgets()));
    netManager->checkCache();
}
void MainWindow::initAppWidgets()
{
    disconnect(netManager, SIGNAL(cacheChecked()),
               this, SLOT(initAppWidgets()));
    initTrayIcon();
    initToolBar();
    initCalendar();
...
void MainWindow::initToolBar()
{
    ...
    connect(toolBar->bookmarkMenu, SIGNAL(triggered(QAction*)),
            this, SLOT(loadBookmarkLink(QAction*)));
    connect(toolBar, SIGNAL(actionRemoved(QString)),
            this, SLOT(removeBookmark(QString)));
    connect(toolBar, SIGNAL(historyItemRemoved(QString)),
            this, SLOT(removeHistoryItem(QString)));
    connect(toolBar->historyMenu, SIGNAL(triggered(QAction*)),
            this, SLOT(loadHistoryLink(QAction*)));
    connect(toolBar->_reloadAction, SIGNAL(triggered()),
            this, SLOT(reloadAction()));
    connect(toolBar->_stopAction, SIGNAL(triggered()),
            this, SLOT(stopAction()));
    connect(toolBar->_forwardAction, SIGNAL(triggered()),
            this, SLOT(forwardAction()));
    connect(toolBar->_backwardAction, SIGNAL(triggered()),
            this, SLOT(backwardAction()));
    connect(toolBar->_zoomUpAction, SIGNAL(triggered()),
            this, SLOT(zoomUpAction()));
    connect(toolBar->_zoomOrigAction, SIGNAL(triggered()),
            this, SLOT(zoomOrigAction()));
    connect(toolBar->_zoomDownAction, SIGNAL(triggered()),
            this, SLOT(zoomDownAction()));
    connect(toolBar->_settingsAction, SIGNAL(triggered()),
            this, SLOT(settingsAction()));
    connect(toolBar->_bookmarkAddAction, SIGNAL(triggered()),
            this, SLOT(addBookmarkDialog()));
    connect(toolBar->_prevDayAction, SIGNAL(triggered()),
            this, SLOT(showPrevDay()));
    connect(toolBar->_nextDayAction, SIGNAL(triggered()),
...
void MainWindow::initCalendar()
{
    ...
    connect(calendarView, SIGNAL(linkClicked(QUrl)),
            this, SLOT(clickedLink(QUrl)));
    connect(calendarView, SIGNAL(loadStarted()),
            statusBar, SLOT(_loadStarted()));
    connect(calendarView, SIGNAL(loadFinished(bool)),
            statusBar, SLOT(_loadFinished(bool)));
    connect(calendarView, SIGNAL(loadProgress(int)),
            statusBar, SLOT(_loadProgress(int)));

    connect(calendarView->_reloadAction, SIGNAL(triggered()),
            this, SLOT(reloadAction()));
    connect(calendarView->_stopAction, SIGNAL(triggered()),
            this, SLOT(stopAction()));
    connect(calendarView->_forwardAction, SIGNAL(triggered()),
            this, SLOT(forwardAction()));
    connect(calendarView->_backwardAction, SIGNAL(triggered()),
            this, SLOT(backwardAction()));
    //connect(calendarView->_settingsAction, SIGNAL(triggered()),
    //          this, SLOT(settingsAction()));
    connect(calendarView->_bookmarkAddAction, SIGNAL(triggered()),
            this, SLOT(addBookmarkDialog()));
    connect(calendarView->_copyAction, SIGNAL(triggered()),
            this, SLOT(copySelectedObject()));
...
QByteArray MainWindow::buildScript()
{
    ...
    if (settingsWidget->showLiturgy()) {
        ...
        param4 = LITURGY1.arg(QString::number(date.dayOfYear()));
    };
...
void MainWindow::reloadCalendar()
{
    ...
    netManager->baseFile->setFileName(QString("")
               .append(netManager->fileTemplate)
               .append("_orthodox_calendar.html"));
...
void MainWindow::addBookmarkAction(QString &bookmarkName)
{
    ...
    if (!keys.contains(bookmarkName) and !bookmarkName.isEmpty()) {
      settingsWidget->settings->setValue(
                  bookmarkName,
                  QVariant(calendarView->url().toString()));
    };
...
void MainWindow::addBookmarkDialog()
{
    ...
    connect(dialog, SIGNAL(bookmarkName(QString&)),
            this, SLOT(addBookmark(QString&)));
...
{
    disconnect(dialog, SIGNAL(bookmarkName(QString&)),
               this, SLOT(addBookmark(QString&)));
    addBookmarkAction(s);
}
